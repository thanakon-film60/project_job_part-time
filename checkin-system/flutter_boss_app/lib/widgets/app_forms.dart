...
/// สีประจำสถานะการติดตาม ใช้ทั้งหมุดบนแผนที่ รายชื่อ และแท็บภาพรวม
/// (STATUS_META ใน frontend/src/pages/LiveMapPage.jsx)
const Map<LiveStatus, Color> liveStatusColors = {
...
Color liveStatusColor(LiveStatus status) =>
    liveStatusColors[status] ?? liveStatusColors[LiveStatus.noData]!;

/// สีของป้ายสถานที่บนปฏิทิน/รายชื่อ — ตรงกับ locationVariant ฝั่งเว็บ
...
class Tag extends StatelessWidget {
  ...
  /// บีบขอบและตัวอักษรให้เล็กลง สำหรับที่แคบมากอย่างช่องวันในปฏิทิน
  /// (กว้างแค่ 1/7 ของจอ — ขอบปกติกินที่จนเหลือข้อความว่า "1 …")
  final bool dense;
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: dense ? 3 : 7, vertical: 2),
      ...
      child: Row(
        ...
        children: [
          ...
          // ป้ายบางอันกว้างกว่าช่องที่มันอยู่ (ช่องวันในปฏิทินกว้างแค่ 1/7 จอ)
          // ต้องย่อข้อความด้วย ... ไม่ใช่ปล่อยให้ Row ล้นจนขึ้นแถบเหลือง-ดำ
          Flexible(
            child: Text(
              text,
              maxLines: 1,
