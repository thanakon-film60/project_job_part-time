...
## ทางลัด — ไม่ต้องแก้เอง
`deploy\windows-server\build-flutter-apk.ps1` เติมสิทธิ์ทั้งหมดในหน้านี้ (ฝั่ง Android),
ตั้ง `minSdkVersion 23`, ตั้งชื่อแอป และสร้างไอคอนจากโลโก้เดียวกับเว็บให้อัตโนมัติ
ก่อน `flutter build apk --release` — ดู [README.md](README.md)

ส่วน iOS ยังต้องแก้ `Info.plist` เองตามหัวข้อด้านบน (build บนเครื่อง Mac เท่านั้น)
