...
**สถานะ: ระบบช่วยเหลือระยะไกลใช้งานได้เต็มรูปแบบบน production แล้ว**

เจ้าของรัน `deploy\cloudflare\enable-support-websocket.ps1` เอง — สคริปต์ทำงานครบทุกขั้น:
หา config ที่ service ใช้จริงเจอ · สำรองไฟล์เดิม · แทรกกฎ · ตรวจไวยากรณ์ผ่าน ·
restart tunnel · เว็บยังตอบ 200

**ผลตรวจหลังเปิดใช้**

```
[ผ่าน] WebSocket ตรงไป backend        — ready role=guest
[ไม่ผ่าน] WebSocket ผ่าน IIS           — missing Connection header  (ตามคาด ยังไม่ได้ reboot)
[ผ่าน] WebSocket ผ่าน thanakronpart-time.com — ready role=guest
```

เส้นทางที่ผู้ใช้จริงเดินผ่านได้แล้ว ส่วน IIS ยังส่งต่อไม่ได้ตามเดิม (ไม่กระทบอะไร
เพราะ tunnel ข้าม IIS ไปเลยสำหรับ path นี้) — ถ้าวันหลัง reboot ตามรอบปกติ
IIS จะส่งต่อได้ด้วย จะลบกฎ cloudflared ทิ้งหรือเก็บไว้ก็ได้

**🔴 บั๊กที่ 4 (ในตัวตรวจของตัวเอง) — เรียกสคริปต์จากโฟลเดอร์อื่นแล้วต่อ DB ไม่ได้**

`verify_support_ws.py` ล้มด้วย `password authentication failed for user "checkin"`
ทั้งที่ production ใช้งานได้ปกติ สาเหตุ: **pydantic-settings อ่าน `.env` เทียบกับโฟลเดอร์
ปัจจุบัน ไม่ใช่ที่อยู่ของไฟล์** พอ ps1 เรียกจาก `deploy\cloudflare` มันหา `.env` ไม่เจอ
แล้วถอยไปใช้ค่า default ทั้งชุด รวมถึงรหัสผ่าน DB ที่เป็นค่าตัวอย่าง

แก้ด้วย `os.chdir(HERE)` ก่อน import `app.config` ในทั้ง `verify_support_ws.py`
และ `test_support_production.py` — เป็นกับดักที่สคริปต์อื่นในโปรเจ็กต์โดนได้เหมือนกัน
ถ้าวันหลังมีคนเรียกจาก path อื่น

**ไฟล์ใหม่: เทสบน production จริง**

| ไฟล์ | หน้าที่ |
| --- | --- |
| `backend/test_support_production.py` | ออกโทเค็นจากบัญชีจริง เรียก node แล้วเก็บกวาดห้องทดสอบ |
| `frontend/tests/support-production.cjs` | ขับเบราว์เซอร์สองฝั่งยิงโดเมนจริง |

เดินเส้นทางเดียวกับผู้ใช้จริงทุกขั้น (Cloudflare → tunnel → backend) จึงเป็นตัวเดียว
ที่พิสูจน์ว่า "เปิดใช้จริงแล้วใช้งานได้" ไม่ใช่แค่ "โค้ดถูก" — **ผ่านครบ 7 ข้อ**:

สร้างห้องบนโดเมนจริง → ผู้ใช้เปิดลิงก์จริงและกดอนุญาตกล้อง → WebRTC ต่อสายสำเร็จ →
ภาพจริงมาถึง 640x480 → เส้นที่วาดวิ่งข้ามโดเมนไปถึง → ภาพนิ่งส่งผ่าน WebSocket ได้
(ข้อความใหญ่ไม่ถูกตัด) → วาดทับภาพนิ่งแล้วตรงจุดเดียวกัน → ปิดห้องแล้วกล้องดับ

ห้องทดสอบถูกลบทิ้งทุกครั้งแม้เทสพังกลางทาง (`support_sessions` เหลือ 0 แถว)
ภาพหน้าจอที่ `backend/storage/logs/production-{host,guest}.png` เห็นลูกศรแดง
พร้อมหัวลูกศรชี้ตรงวัตถุบนภาพนิ่ง — วาดจากจอคอมแล้วไปโผล่ตรงตำแหน่งเดียวกันบนมือถือ

**สรุปบั๊กทั้งหมดที่เจอในฟีเจอร์นี้: 4 ตัว** — เทสฝั่งเซิร์ฟเวอร์ 79 ข้อจับได้ 0 ตัว
เทสเบราว์เซอร์จับได้ 3 ตัว และเทสบน production จับได้อีก 1 ตัว
บทเรียน: ฟีเจอร์ที่พึ่ง WebRTC/สื่อจริง ต้องมีเทสที่เปิดเบราว์เซอร์จริง ไม่งั้นมองไม่เห็นอะไรเลย

**ยังไม่ได้ทำ:** ยังไม่ได้ `git push` · ยังไม่ได้ตั้ง TURN (STUN อย่างเดียวต่อติดราว 80-90%
ของเน็ตทั่วไป ถ้าเจอเคสต่อไม่ติดบ่อยค่อยเช่า coturn) · ฟีเจอร์ `Web-WebSockets` ของ IIS
ยังค้าง `InstallPending` (ไม่จำเป็นแล้ว แต่ reboot รอบหน้าจะติดตั้งเสร็จเอง)
