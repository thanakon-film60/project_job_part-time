...
### 2026-09-13 — UI สแกนใบหน้าแบบมีกรอบวงรี + ตรวจท่าทางจริง

**โจทย์:** ทำ UI ตามภาพตัวอย่าง — ใบหน้าต้องอยู่ในกรอบวงรี มีคำสั่ง "กระพริบตา" และนับถอยหลัง

**🔴 บั๊ก production ที่เจอระหว่างทาง — ร้ายแรงที่สุดของรอบนี้**

`app/config.py` **ไม่มีค่า `home_verification_*` เลย** (แก้ค้างไว้รอบก่อนแล้วไม่ได้ commit
หายแบบเดียวกับที่ `web.config` หาย) แต่ `routers/home_verifications.py` เรียกใช้ 6 จุด

ผลคือ backend ที่ deploy ไปแล้วจะ **`AttributeError` → HTTP 500 ทันทีที่มีคนกดยืนยันจริง**
การ probe ด้วย curl ไม่เจอเพราะได้ 401 จาก auth ก่อนเข้าโค้ด — และนี่คือสาเหตุที่หน้าจอ
ค้างอยู่ที่ "กำลังเตรียมการยืนยัน..." ตอนทดสอบ **คืนค่าทั้ง 3 ตัวกลับไปแล้ว**

> บทเรียน: probe ที่ได้ 401 **ไม่ได้แปลว่า endpoint ทำงานได้** มันแค่แปลว่า routing ถูก
> ต้องทดสอบด้วย token จริงถึงจะรู้ว่าโค้ดในตัว handler รันผ่าน

**ไฟล์ใหม่ (ทั้ง 2 แอป)**

- `lib/services/face_framing.dart` — ตรรกะบริสุทธิ์ทั้งหมด แยกจาก UI เพื่อเขียนเทสต์ได้โดยไม่ต้องมีกล้อง
  - `FaceFrameTarget` — วงรีเก็บเป็น**สัดส่วน 0..1** ไม่ใช่พิกเซล เพื่อให้กรอบที่ผู้ใช้เห็น
    กับกรอบที่ระบบตรวจใช้ตัวเลขชุดเดียวกัน (ถ้าใช้พิกเซลจะหลุดจากกันทันที)
  - `evaluateFraming()` — ไล่ตรวจ ไม่มีหน้า → ระยะ → ตำแหน่ง → ความเอียง ให้แก้ทีละอย่าง
  - `FaceChallenge` — state machine ตรวจท่าจริง

**ไฟล์ที่แก้**

- `lib/services/face_service.dart` — เปลี่ยนจากคืน `(bool, bool)` เป็น `FaceObservation`
  ที่มี boundingBox / eye probability / yaw / roll ครบ · **สลับ width-height เมื่อหมุน 90/270°**
  ไม่งั้นสัดส่วนที่คำนวณจะผิดแกน
- `lib/widgets/face_scanner.dart` — เขียนใหม่: หน้ากากทึบเจาะวงรีด้วย `Path.combine(difference)`,
  ขอบเปลี่ยนสีตามสถานะ, ส่วนโค้งบอกความคืบหน้า, นับถอยหลัง 3 วิแล้วถ่ายเอง
- `lib/models/home_verification.dart` + `screens/home_verification_screen.dart` — ส่ง `actionCode` ต่อให้ scanner
- `backend/app/home_verification.py` — `CHALLENGE_ACTIONS` เปลี่ยนเป็นคู่ (รหัส, ข้อความ)
- `backend/app/routers/home_verifications.py` — ส่ง `action_code` เพิ่มใน response

**🔴 แก้ช่องโหว่ liveness ของเดิม**

ของเดิมเขียนว่า `(leftEye > 0.4 || rightEye > 0.4)` = **"ตาเปิดอยู่"** เฉย ๆ
แปลว่า **เอารูปถ่ายที่ลืมตามาส่องก็ผ่าน** ทั้งที่หน้าจอเขียนว่า "กรุณากะพริบตา"

ตอนนี้บังคับให้เห็น**การเปลี่ยนแปลงตามเวลา**: กะพริบต้องเห็น เปิด→ปิด→เปิด ครบรอบ ·
หันหน้าต้องเห็นหน้าตรงก่อนแล้วค่อยหัน (กันเอารูปที่ถ่ายเอียงไว้มาส่อง)
มีเทสต์ชื่อ "รูปถ่ายลืมตาต้องไม่ผ่าน" คุมไว้โดยเฉพาะ

> ⚠️ ยังกันได้แค่**ภาพนิ่ง** วิดีโอหรือหน้ากากยังผ่านได้ และ server ยังไม่ได้ตรวจว่าเป็นใคร

**เรื่องทิศทางการหันหน้า:** เครื่องหมายของ `headEulerAngleY` ขึ้นกับกล้องหน้า/หลังและการ mirror
ของ preview จึง**ไม่ผูกทิศตายตัว** — รับว่า "หันแล้ว" เมื่อเกินองศาที่กำหนด ดีกว่าผูกทิศแล้ว
ผู้ใช้หันถูกแต่ระบบไม่ผ่านโดยไม่มีใครรู้สาเหตุ

**ความเข้ากันได้:** `challengeAction` เป็น optional — หน้าลงทะเบียนใบหน้าและหน้าเช็คอินงาน
ที่ใช้ `FaceScanner` ร่วมกันได้พฤติกรรมเดิม (แค่เพิ่มกรอบวงรีช่วยจัดตำแหน่ง) ·
`action_code` ก็ optional ฝั่งแอป ถ้า backend ไม่ส่งมาจะถอยไปเดาจากข้อความไทย

**ไม่ได้ลงไลบรารีเพิ่มเลย** — `camera` + `google_mlkit_face_detection` ที่มีอยู่ให้ข้อมูลครบแล้ว
วงรีใช้ `CustomPainter` ธรรมดา (พิจารณา `face_camera` แล้วแต่ไม่ใช้ เพราะมันครอบ flow ทั้งหมด
จนยัด challenge จาก server ไม่ได้ และเสี่ยงชน AGP ที่โปรเจ็กต์นี้ตรึงเวอร์ชันไว้)

**ผลทดสอบ:** analyze สะอาดทั้งคู่ · backend **31 tests** · พนักงาน **139 tests** · หัวหน้า **178 tests**
(เพิ่มฝั่งละ 33) · build ผ่านทั้งคู่ · `detect-changes` risk **low** affected processes **0**

**bump version:** พนักงาน `1.6.1+9` → `1.7.0+10` · หัวหน้า `1.3.1+6` → `1.4.0+7`

**ยังค้าง:** **ต้อง deploy backend ใหม่** (ค่า config ที่หายไป + `action_code`) · ทดสอบ UI บนเครื่องจริง
(มือถือล็อกหน้าจออยู่ตอนทดสอบ) · publish APK
